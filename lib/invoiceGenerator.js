...
export async function generateInvoice(order) {
  ...
  try {
    ...
    // Configure browser settings based on environment
    const browserConfig = isProd
      ? {
          args: [
            ...chromium.args,
            "--no-sandbox",
    ...
    // Set content and wait for network and DOM to be ready
    await page.setContent(invoiceHtml, {
      waitUntil: ["networkidle0", "load", "domcontentloaded"],
    });

    // Wait for fonts to load using evaluation
    await page.evaluate(() => {
...
// Example usage in serverless environment:
/*
